...
#define fluid_cond_mutex_init(m)      mtx_init(m, mtx_plain)
#define fluid_cond_mutex_destroy(m)   mtx_destroy(m)
...
{
    fluid_cond_mutex_t *mutex;
    mutex = calloc(1, sizeof(fluid_cond_mutex_t));
    fluid_cond_mutex_init(mutex);
    return mutex;
}
...
#define fluid_cond_init(cond)           cnd_init(cond)
#define fluid_cond_destroy(cond)        cnd_destroy(cond)
...
/* Thread private data (faked with a thread local void* array here) */

#define FAKE_TSS_MAX 256

extern __thread void *fake_tss[FAKE_TSS_MAX];

static FLUID_INLINE void *
fake_tss_get(tss_t tss_id)
{
    if (tss_id >= FAKE_TSS_MAX)
        return NULL;
    return fake_tss[tss_id];
    // return tss_get(tss_id);
}

static FLUID_INLINE int
fake_tss_set(tss_t tss_id, void *val)
{
    if (tss_id >= FAKE_TSS_MAX)
        return -1;
    fake_tss[tss_id] = val;
...
int fake_tss_create(tss_t *key);
int fake_tss_delete(tss_t key);

typedef tss_t fluid_private_t;
#define fluid_private_init(_priv)                  fake_tss_create(&_priv)
#define fluid_private_free(_priv)                  fake_tss_delete(_priv)
#define fluid_private_get(_priv)                   fake_tss_get((_priv))
#define fluid_private_set(_priv, _data)            fake_tss_set((_priv), (_data))
